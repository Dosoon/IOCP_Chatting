...
void PacketManager::Init(RoomManager* p_ref_room_manager, UserManager* p_ref_user_manager, RedisManager* p_ref_redis_manager)
{
	SetManagers(p_ref_room_manager, p_ref_user_manager, p_ref_redis_manager);

	p_ref_user_manager_->Init();
	p_ref_room_manager_->Init();
...
void PacketManager::Start(RoomManager* p_ref_room_manager, UserManager* p_ref_user_manager, RedisManager* p_ref_redis_manager)
{
	Init(p_ref_room_manager, p_ref_user_manager, p_ref_redis_manager);
	Run();
}
...
bool PacketManager::ProcessPacket(PacketInfo& pkt, bool user_pkt)
{
	if (packet_handlers_.contains(pkt.id_)) {
		(this->*(packet_handlers_[pkt.id_]))(pkt.session_index_, pkt.size_, pkt.data_);

		if (user_pkt) {
			if (p_ref_user_manager_->CompleteProcess(pkt.session_index_, pkt.size_) == false) {
				incoming_packet_user_index_queue_.push(pkt.session_index_);
			}
		}
		return true;
	}
...
void PacketManager::ConnectHandler(uint32_t session_idx, uint16_t, char*)
{
...
void PacketManager::DisconnectHandler(uint32_t session_idx, uint16_t, char*)
{
	auto p_user = p_ref_user_manager_->GetUserByIndex(session_idx);
...
void PacketManager::EnterRoomHandler(uint32_t session_idx, uint16_t, char* p_data)
{
...
void PacketManager::LeaveRoomHandler(uint32_t session_idx, uint16_t, char* p_data)
{
...
void PacketManager::RoomChatHandler(uint32_t session_idx, uint16_t, char* p_data)
{
